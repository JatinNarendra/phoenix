...
export const useCheckReferral = () => {
  const { checkUserReferral, resetReferralCheck, showReferralPopup, handleCloseReferralPopup } = useReferral();
  const user = useUser();
  ...
  useEffect(() => {
    if (user.id && !user.isLoading) {
      console.log('[REFERRAL DEBUG] useCheckReferral effect triggered, user ID:', user.id);
      ...
      if (typeof window !== 'undefined') {
        ...
        if (window.Telegram?.WebApp) {
          try {
            ...
            const webAppData = window.Telegram.WebApp as unknown as TelegramWebAppExtended;
            const initDataUnsafe = webAppData.initDataUnsafe || {};
            
            // Check for start_param
            const telegramStartParam = initDataUnsafe.start_param;
            
            console.log('[REFERRAL DEBUG] Telegram start_param:', telegramStartParam);
            console.log('[REFERRAL DEBUG] Full initDataUnsafe:', JSON.stringify(initDataUnsafe));
            
            if (telegramStartParam) {
              console.log('[REFERRAL DEBUG] Found valid Telegram start_param:', telegramStartParam);
              ...
              const timer = setTimeout(() => {
                ...
                console.log('[REFERRAL DEBUG] Checking user referral after Telegram start_param');
                checkUserReferral();
              }, 100);
            ...
            console.error("[REFERRAL DEBUG] Error accessing Telegram WebApp parameters:", e);
          }
        } else {
        ...
        if (startappParam) {
          console.log('[REFERRAL DEBUG] Found startapp param in URL:', startappParam);
          ...
          const timer = setTimeout(() => {
            paramsCheckedRef.current = true;
            console.log('[REFERRAL DEBUG] Checking user referral after URL startapp param');
            checkUserReferral();
          }, 100);
        ...
        console.log('[REFERRAL DEBUG] Saved startapp parameter:', savedStartappParam);
        ...
        if (refParam || startappParam || refDirectParam || urlStartParam || savedStartappParam) {
          console.log('[REFERRAL DEBUG] Referral parameter found, resetting check');
          resetReferralCheck();
          
          // Small delay to ensure the reset has taken effect
          const timer = setTimeout(() => {
            paramsCheckedRef.current = true;
            console.log('[REFERRAL DEBUG] Checking user referral after found parameter');
            checkUserReferral();
          }, 100);
          
          return () => clearTimeout(timer);
        } else if (!paramsCheckedRef.current) {
          console.log('[REFERRAL DEBUG] No specific referral parameters, doing regular check');
  ...
  }, [user.id, user.isLoading, searchParams, checkUserReferral, resetReferralCheck]);
