...
interface GameToastOptions extends Omit<ToastOptions, "icon"> {
...
export const showGameToast = (
  ...
  const variantClass = toastOptions?.variant
    ? `game-toast-${toastOptions.variant}`
    : "";

  // Create the new toast
  const { icon, ...restOptions } = toastOptions || {};
...
export const gameToast = {
  success: (
    message: string | React.ReactNode,
    options?: Partial<GameToastOptions>
