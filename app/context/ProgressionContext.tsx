...
interface ProgressionContextType {
  ...
  getTimeRemaining: (nextRotationTime: number) => {
    hours: number;
    minutes: number;
    seconds: number;
    total: number;
  ...
  getTimeUntilTypeActive: (type: number) => {
    hours: number;
    minutes: number;
    seconds: number;
    total: number;
  };
  getTimeUntilTypeEnd: (type: number) => {
    hours: number;
    minutes: number;
    seconds: number;
    total: number;
...
export const getTypeSchedule = (type: number) => {
  ...
  if (typeIndex === currentTypeIndex) {
    ...
    typeStartTime =
      GLOBAL_ROTATION_BASE_TIME + rotationCycles * ROTATION_DURATION;
    ...
    typeStartTime =
      GLOBAL_ROTATION_BASE_TIME +
      (rotationCycles - cyclesAgo) * ROTATION_DURATION;
    typeEndTime = typeStartTime + ROTATION_DURATION;
  } else {
    ...
    typeStartTime =
      GLOBAL_ROTATION_BASE_TIME +
      (rotationCycles + cyclesAhead) * ROTATION_DURATION;
    typeEndTime = typeStartTime + ROTATION_DURATION;
  }
...
export const getNextTokenType = (currentType: number): number => {
  const currentIndex = rotationSequence.findIndex(
...
export const getGlobalRotationInfo = () => {
  ...
  const nextRotationTime =
    GLOBAL_ROTATION_BASE_TIME + (rotationCycles + 1) * ROTATION_DURATION;
...
export const ProgressionProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [state, setState] = useState<ProgressionState>(() => {
    ...
    if (gameState.spinProgression) {
      ...
      const initialState = {
        currentType: newType,
        currentStep:
          shouldResetStep || shouldResetDueToInvalidStep
            ? 0
            : gameState.spinProgression.currentStep,
        collectedTokens:
          shouldResetStep || shouldResetDueToInvalidStep
            ? 0
            : gameState.spinProgression.collectedTokens,
        requiredTokens:
          shouldResetStep || shouldResetDueToInvalidStep
            ? allProgressionData[currentlyActiveType][0].tokens
            : gameState.spinProgression.requiredTokens,
        reward:
          shouldResetStep || shouldResetDueToInvalidStep
            ? allProgressionData[currentlyActiveType][0].reward
            : gameState.spinProgression.reward,
  ...
  const syncToGameState = useCallback(
    async (newState: ProgressionState) => {
...
export const useProgression = (): ProgressionContextType => {
  ...
  if (context === undefined) {
    if (
      ...
      console.warn(
        "useProgression called on Nexus route without ProgressionProvider, returning null context"
      );
