...
export default function GameLayout({
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const productionResult = isSparkyDomain || (isVercelApp && isHTTPS) || (isHTTPS && !hostname.includes('localhost'));
  ...
  if (isNexus) {
    console.log('GameLayout: Bypassing GameLayout for Nexus route');
    return null;
  }
  
  // For all other routes, render full game layout
  console.log('GameLayout: Rendering full game layout for non-Nexus route');
  return (
    <ErrorBoundary>
      <GameProviders>
        <Toaster
