...
const PhoenixParticles = ({ newLevel }: { newLevel: number }) => {
  ...
  useEffect(() => {
    ...
    const sparkIncrease = currentLevelConfig.sparkRequired - prevLevelConfig.sparkRequired;
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      positions[i * 3 + 1] = radius * Math.sin(phi) * Math.sin(theta) - 2 + spiralOffset;
      ...
      colors[i * 3] = selectedColor.r * baseIntensity * (intensityBase + Math.random() * 0.4 + levelBoost);
      colors[i * 3 + 1] = selectedColor.g * baseIntensity * (intensityBase + Math.random() * 0.4 + levelBoost);
      colors[i * 3 + 2] = selectedColor.b * baseIntensity * (intensityBase + Math.random() * 0.4 + levelBoost);
    ...
    particleGeometryRef.current.setAttribute('color', new THREE.BufferAttribute(colors, 3));
    particleGeometryRef.current.setAttribute('scale', new THREE.BufferAttribute(scales, 1));
  }, [newLevel]);
...
const PhoenixLevelUpLoader: React.FC<PhoenixLevelUpLoaderProps> = ({
  ...
  useEffect(() => {
    ...
    backgroundImage.src = getPhoenixImage(newLevel);
    badgeImage.src = getBadgeImage(newLevel);

    console.log('Loading images:', {
      background: backgroundImage.src,
      badge: badgeImage.src
    });
    ...
    badgeImage.onload = () => {
      console.log('Badge image loaded');
      setBadgeLoaded(true);
    };
    ...
    badgeImage.onerror = (e) => {
      console.error('Failed to load badge image:', e);
    };
  }, [newLevel]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <div className="text-center z-20">
          <div className="leveling-up-text">
            Leveling Up...
          </div>
          <div className="new-level-text">
            Level {newLevel}
          </div>
      ...
      <style jsx>{`
        ...
        .new-level-text {
          ...
          font-size: 1.8rem;
          font-weight: bold;
          background: linear-gradient(to bottom, #fff 5%, #ffd700 40%, #ff4800 60%);
        ...
        @keyframes flameFlicker {
          ...
          25% { opacity: 0.9; transform: scale(0.98) rotate(-0.5deg); }
          50% { opacity: 1; transform: scale(1.02) rotate(0.5deg); }
          75% { opacity: 0.9; transform: scale(0.99) rotate(-0.25deg); }
        }
